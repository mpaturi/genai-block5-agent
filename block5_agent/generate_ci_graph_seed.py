...
Everything above is derived from answer_key.json and rag_fixtures.json,
so it can only ever reproduce a count graph_tool.py's own verify+count
logic already computed - it can't catch a real bug in that logic (an AND
that regressed to an OR, the wrong lab property, a dropped clause), since
the seed would just be regenerated to match whatever the buggy code
produces. _EDGE_CASE_PERSON_ID below is the one exception: a hand-written
patient, not derived from any real RAG result or answer, who genuinely
has an answerable task's condition but is given a lab value that
deliberately fails its comparison, and carries a drug name used nowhere
else in the seed. It's injected directly into patient_lab_values and the
other seen-dicts below, after the shared-patient/combined-constraint
machinery above has already run - never routed through that machinery -
so it's unaffected by, and doesn't affect, which real patients happen to
be shared across tasks. Every real, RAG-derived patient ID across all of
data/eval/rag_fixtures.json falls between 18 and 11557.
_EDGE_CASE_PERSON_ID is deliberately far outside that range so it's
obviously synthetic, never mistaken for a real patient later. See
tests/test_graph_tool_integration.py for the test this fixture exists to
support.
"""
import json
...
def main() -> None:
    ...
    # See the module docstring: this one patient is hand-written, not
    # derived from any real RAG result or answer, precisely so it can
    # catch a bug the derivation above never could. Injected directly
    # into patient_lab_values (after it's already been fully resolved
    # above) rather than into patient_lab_constraints, since that
    # constraint-resolution machinery only ever computes values that
    # satisfy every sharing task's comparison - there's no way to ask it
    # for a deliberately failing value.
    ...
    patient_lab_values[_EDGE_CASE_PERSON_ID] = {edge_case_lab_property: _EDGE_CASE_LAB_VALUE}
