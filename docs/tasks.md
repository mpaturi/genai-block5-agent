...
## Phase 17 — Field sanitization (`phase-17-field-sanitization`, base: `main`)

Closes the gap left after Phase 15's plausibility check: a `condition`/`lab`/`drug_a`/`drug_b` value can be a real, valid-looking string that still carries structural conversation-turn markers (role prefixes, chat-template delimiters) — plausibility flags an *unrecognized* value, but says nothing about markers embedded inside an otherwise-real one, and nothing was stripping them before they reached an LLM prompt.

- [x] **Added `sanitize_field()` to `schemas.py`.** Same `_ROLE_MARKER_RE`/`_CHAT_DELIMITER_RE` patterns, same "replace with a single space, not empty string" reasoning, as `genai-block4-rag-eval/scripts/sanitize.py`'s `sanitize_chunk_text()` — one shared implementation, not a second copy that could drift. Public, not underscore-prefixed, since `agent.py` needs to import it too. Docstring notes explicitly: never applied before `check_plausibility()` sees these same four fields — that check's exact-match logic and flag messages need the caller's real, unmodified value.
- [x] **Applied `sanitize_field()` at both places these fields reach an LLM prompt or the RAG query text.** `build_rag_query()` (condition/lab) and `assemble_question_text()` (condition/lab/drug_a/drug_b) in `schemas.py`. `_default_answer_fn()`'s prompt (`agent.py`) also directly re-embeds `question.drug_a`/`question.drug_b` on its own "count:" lines, independent of whatever `assemble_question_text()` returns elsewhere in the same prompt — this was the one gap that didn't route through the shared function, closed by importing `sanitize_field()` into `agent.py` and applying it there too. `run_agent()`'s `plausibility_check_fn` call is deliberately untouched — that check still needs the caller's real value.
- [x] **Tests:** unit coverage for `sanitize_field()` itself (mirroring `genai-block4-rag-eval/tests/test_sanitize.py`), `build_rag_query()`/`assemble_question_text()` clean-output tests for a planted injection in condition/lab/drug_a/drug_b, and a new `tests/test_default_answer_fn.py` exercising `_default_answer_fn()`'s real prompt directly (not through the stubbed `answer_fn` every other agent test uses) — mirrors `genai-block4-rag-eval/tests/test_generate.py`'s `_FakeAnthropicClient`/`_FakeMessages` pattern (`Anthropic`/`wrap_anthropic` monkeypatched, memoized `_anthropic_client` reset), the one test that specifically proves the `agent.py` gap is closed, not just `schemas.py`'s. Confirmed `tests/test_agent_plausibility.py` passes unchanged — its fakes ignore `condition`/`lab`/`drug_a`/`drug_b` entirely, so this fix was never expected to touch it, checked directly rather than assumed.
- [x] 105 passed, 1 skipped (live-data integration test) — full suite
- [x] Push `phase-17-field-sanitization`, open PR against `main`
