...
| `scripts/build_eval_answer_key.py` | reads `data/eval/tasks.json` and writes `data/eval/answer_key.json` — the correct patient list, drug count, and expected `confidence` for each answerable question, keyed by the same question ID, worked out once, ahead of time by making its own throwaway calls straight to the live search service and live graph, since `rag_tool.py`/`graph_tool.py` don't exist yet at this point; imports the shared query-building function from `scripts/schemas.py` (never a second, separately written version of it) to build the search-service call, so the golden patient list matches what the agent actually produces; the raw result is deduped and ordered using Tool 1's exact rule (score descending, ties broken by patient ID) before being written, so the exact-match check on `rag_patient_ids` (an ordered list) is comparing against a golden list built the same way the agent builds its own |
...
| `scripts/rag_tool.py` | semantic search tool; imports the shared query-building function from `scripts/schemas.py` (the same one `build_eval_answer_key.py` uses) rather than reformatting `condition`/`lab`/`comparison`/`value` on its own |
