...
| `scripts/build_eval_answer_key.py` | makes its own throwaway calls straight to the live search service and live graph, since `rag_tool.py`/`graph_tool.py` don't exist yet at this point, and writes the result to `data/eval/answer_key.json` |
| `data/eval/answer_key.json` | the correct answer for each answerable question, keyed by the same question ID used in `tasks.json` — generated by `build_eval_answer_key.py`, not hand-written |
...
| `scripts/agent.py` | the agent itself — the steps described in `docs/spec.md`'s Agent steps section, built so the two tools can be swapped for fakes in tests |
